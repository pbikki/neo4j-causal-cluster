...
if [ -z $REMOTE_BACKUP_FILE ]; then
    echo "You must specify a BACKUP_FILE such as s3://my-backups/my-backup.tar.gz"
    exit 1
fi
if [ -z $DATABASE ]; then
...
echo "REMOTE_BACKUP_FILE=$REMOTE_BACKUP_FILE"
...
if [ -d "/data/databases/$DATABASE" ] ; then
    echo "You have an existing graph database at /data/databases/$DATABASE"
    ...
    echo "No existing graph database found at /data/databases/$DATABASE"
fi
...
echo "Copying $REMOTE_BACKUP_FILE -> $RESTORE_ROOT"
...
# gsutil -m cp -r "$REMOTE_BACKUP_FILE" "$RESTORE_ROOT"
aws --endpoint-url=$COS_ENDPOINT s3 cp "$REMOTE_BACKUP_FILE" "$RESTORE_ROOT"
...
BACKUP_FILENAME=$(basename "$REMOTE_BACKUP_FILE")
...
if [[ $BACKUP_FILENAME =~ \.tar\.gz$ ]] ; then
    ...
    if [ -z $BACKUP_SET_DIR ] ; then
        ...
        RESTORE_FROM="$RESTORE_ROOT/data/$DATABASE"
    else 
    ...
    if [ -z $BACKUP_SET_DIR ] ; then
        echo "BACKUP_SET_DIR was not specified, so I am assuming this backup set was formatted by my backup utility"
        RESTORE_FROM="$RESTORE_ROOT/$DATABASE"
    else
        RESTORE_FROM="$RESTORE_ROOT/$BACKUP_SET_DIR"
    fi
else
...
if [ -d "/data/databases/$DATABASE" ] ; then
   if [ "$FORCE_OVERWRITE" = "true" ] ; then
      ...
      rm -rf /data/databases/$DATABASE
   fi
fi

mv /var/lib/neo4j/data/databases/$DATABASE /data/databases/
...
ls -al /data/databases/$DATABASE

echo "Final size"
du -hs /data/databases/$DATABASE
