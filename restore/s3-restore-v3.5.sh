...
if [[ $BACKUP_FILENAME =~ \.tar\.gz$ ]] ; then
    ...
    if [ -z $BACKUP_SET_DIR ] ; then
        echo "BACKUP_SET_DIR was not specified, so I am assuming this backup set was formatted by my backup utility"
        RESTORE_FROM="$RESTORE_ROOT/$UNZIPPED_BACKUP_DIR"
    else
        RESTORE_FROM="$RESTORE_ROOT/$BACKUP_SET_DIR"
    fi
else
...
echo "Debugging directory sizes before restore"
du -hs "/data"
ls -al "/data"
...
echo "Debugging directory sizes after restore"
du -hs "/data"
ls -al "/data"
...
echo "Debugging directory sizes after restore and moving files to /data/databases/"
du -hs "/data"
ls -al "/data"
